...
| 功能 | 稳定版 `v1.14.1.2` | 层叠版 `v1.16.1.2` | 编译版 `v1.16.1.1-full-web` |
|------|-------------------|-------------------|---------------------------|
| 镜像大小 | ~179MB | ~216MB | ~219MB |
| 运行时大小 | ~229MB | ~476MB | ~479MB |
| 构建方式 | 原始构建 | 层叠升级 | 源码编译 |
| ZeroTier 版本 | 1.14.1 | 1.16.1 | 1.16.1 |
| ztncui Web UI | 有 | 有 | 有 |
| 默认密码 | admin/admin | admin/admin | admin/admin |
| 首次登录强制改密 | 否 | 否 | 是 |
| 老用户升级保留密码 | - | 是 | 是 |
| 稳定性 | **最稳定** | 测试中 | 测试中 |
| 适用场景 | **生产环境首选** | 测试/尝鲜 | 测试/尝鲜 |

**⚠️ 重要提示：**
- `v1.16.1.2` 和 `v1.16.1.1-full-web` 升级到 ZeroTier One 1.16.1 核心，目前测试能用，但不确定是否稳定运行
- 如遇到网络反复重新连接或其他 bug，请积极提交 issue 说明
- **如需稳定运行，请使用 `v1.14.1.2` 版本**
...
**从 v1.14.1.2 升级到 v1.16.1.2 或 v1.16.1.1-full-web：**
...
2. **密码处理**：
   ...
   - `v1.16.1.1-full-web` 首次登录后**强制修改密码**（密码长度至少 10 位）
3. **端口映射调整**：
